package com.example.mybatis.codegen.demo;
...
import com.example.mybatis.codegen.demo.db.IDbHelper;
import com.example.mybatis.codegen.demo.exception.CodegenException;
import com.example.mybatis.codegen.demo.model.ConfigModel;
import com.example.mybatis.codegen.demo.model.TableModel;
import com.example.mybatis.codegen.demo.util.FileHelper;
import com.example.mybatis.codegen.demo.util.StringUtil;
import com.example.mybatis.codegen.demo.util.XmlUtil;
...
public class Codegen {
	...
	public ConfigModel loadXml() throws Exception {
		...
		ConfigModel.Templates templates = cm.new Templates(basepath);
		...
		if(filesEl!=null){
			ConfigModel.Files files=cm.new Files();
			cm.setFiles(files);
			...
			for (Iterator i = root.elementIterator("tables"); i.hasNext();) {
				...
				for (int j = 0; j < tableNames.length; j++) {
					ConfigModel.Table table = cm.new Table(tableNames[j]);
			...
			for (Iterator i = root.elementIterator("table"); i.hasNext();) {
				...
				ConfigModel.Table table = cm.new Table(tableName);
		...
		for (Iterator i = root.elementIterator("genAll"); i.hasNext();) {
			...
			ConfigModel.GenAll genAll = cm.new GenAll(tableNames);
			cm.setGenAll(genAll);
			for (Iterator j = tableEl.elementIterator("file"); j.hasNext();) {
				...
				ConfigModel.GenAll.File file = genAll.new File(template, filename, extName, dir, genMode);
